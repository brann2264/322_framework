import generate_interference as interference
import utils
import argparse
import os
...
def test_file(binary_location: str,
    ...
    try:
        result = interference.run_interference(binary_location,
                                               input_file,
                                               timeout)
        ...
        with open(oracle_name, 'r') as ifile:
            expected = interference.parse_interference_results(ifile.read())
...
def test_dir(binary_location: str,
    ...
    for filename in input_files:
        if filename.endswith('.L2f'):
            ...
            if test_count > 0 and test_count % 100 == 0:
